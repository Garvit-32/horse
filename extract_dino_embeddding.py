...
def parse_args():
    ...
    parser.add_argument('--dino_batch_size', type=int, default=96, help="Batch size for DINO inference")
...
# --- 6. Perform Batched DINO Inference ---
    ...
    try:
        for i in range(num_batches):
            ...
            all_dino_preds.append(embeddings)
            all_idxs.append(batch_indices)
        ...
        flattened_idxs = np.concatenate([np.array(x, dtype=np.int32).ravel() for x in all_idxs])
        final_results['frame_idx'] = torch.tensor(flattened_idxs)
        final_results['x_norm_clstoken']  = torch.cat(all_dino_preds, dim=0).cpu()


        dir_name = os.path.dirname(video_path).replace('dataset', 'dino_tensor')
        os.makedirs(dir_name, exist_ok=True)
        save_path = video_path.replace('dataset', 'dino_tensor').replace('.mp4', '_results.pt')
        torch.save(final_results, save_path)

        del all_idxs, all_dino_preds
...
if __name__ == "__main__":
    ...
    num_processes = 1 # Number of subprocesses to use - as requested
