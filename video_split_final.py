...
def process_video(tensor_file_path: str):
    ...
    # ------------------------------------------------------------------
    # 7-c. Write one video per smoothed segment and save dinop embeddings
    # ------------------------------------------------------------------
    orientation_counts: dict[str, int] = collections.defaultdict(int)
    frame_ptr = 0                                         # where we are in needed_frame_indices
    ...
    # ------------------------------------------------------------------
    if ARGS.debug:
        dbg_path   = f"{output_base}_debug_full.mp4"
        dbg_writer = cv2.VideoWriter(
            dbg_path, fourcc, ARGS.output_fps, (output_width, output_height)
        )

        # expand segment list into one label per frame
        flat_labels = [d for d, n in smoothed_segments for _ in range(n)]

        for k, idx in enumerate(needed_frame_indices):
            frame = frames_dict[idx]
            if frame.shape[:2] != (output_height, output_width):
                frame = cv2.resize(frame, (output_width, output_height))

            label = flat_labels[k] if k < len(flat_labels) else "unknown"
        ...
        dbg_writer.release()
        print(f"✓ full debug video saved                → {dbg_path}")
