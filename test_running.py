...
def parse_args():
    ...
    parser.add_argument('--dino_batch_size', type=int, default=16, help="Batch size for DINO inference")
...
def predict_orientation_dino_batch(embeddings_batch, prototypes):
    ...
    for i in range(embeddings_batch.shape[0]):
        ...
        running = scores_dict.get('running', -1) > (scores_dict.get('right', -1) + 
                                                    scores_dict.get('left', -1) + 
                                                    scores_dict.get('front', -1) + 
                                                    scores_dict.get('back', -1))
                                                     
         # Example thresholdscores_dict.get('running', -1)
...
def process_video(video_path: str):
    ...
    # try:
        ...
        # json_path = video_path.replace('data', 'data_cropped').replace('_results.pt', '_seg.json')
    ...
    while True:
        ...
        if current_frame_idx % 3 == 0:
            frames_dict[current_frame_idx] = frame
    ...
    for frm_idx, frame in frames_dict.items():
    ...
    # output_file = video_path.replace('running', 'running_test/running')
    ...
    # orientations = ['running', 'not_running'] # Add unknown if needed
    ...
    for frame_idx in frames_dict.keys():
        dino_pred = all_dino_preds[frm_idx]
        running_or_not = dino_pred['running']
        temp.append(running_or_not)
        score_dict = dino_pred['all_scores']
        
    detected = mode(temp)
    
    print(detected , video_path)
    
    if detected == True:
        out_path = video_path.replace('running', 'running_test/running')
        output_dir_name = os.path.dirname(out_path)
        os.makedirs(output_dir_name, exist_ok=True)
        output_writer = cv2.VideoWriter(out_path, fourcc, ARGS.output_fps, (output_width, output_height))
        write_frame = cv2.resize(frame, (output_width, output_height), interpolation=cv2.INTER_LINEAR)
        for frame in frames_dict.values():
            write_frame = cv2.resize(frame, (output_width, output_height), interpolation=cv2.INTER_LINEAR)
            output_writer.write(write_frame)

    else:
        out_path = video_path.replace('running', 'running_test/not_running')
        output_dir_name = os.path.dirname(out_path)
        os.makedirs(output_dir_name, exist_ok=True)
        output_writer = cv2.VideoWriter(out_path, fourcc, ARGS.output_fps, (output_width, output_height))
        write_frame = cv2.resize(frame, (output_width, output_height), interpolation=cv2.INTER_LINEAR)
...
if __name__ == "__main__":
    ...
    # --- Setup ---
    video_root = os.path.join(args.data_dir, 'running')
    video_files = sorted(list_mp4_files(video_root))
